--------------------------一.LUA简介--------------------------
lua是一种弱类型的动态脚本语言,提供了一个虚拟机;lua代码最终都是以字节码的形式有虚拟机解释执行的.
把外部组织好的代码置入内存,让虚拟机解析运行,需要有一个源代码解释器,或者预编译字节码的加载器. 
Lua不能独立运行;

解释型语言:程序不需要编译,程序在运行时才翻译成机器语言,每执行一次都要翻译一次.因此效率较低;但同时依赖解释器,跨平台性好; 
官方源码大致分为四部分:
1.虚拟机运转的核心功能
2.源代码解析以及预编译字节码
3.内嵌库
...
1.nil		-- 空 既是类型,也是值;删除某个变量,只需赋值nil即可; 所有未初始化的变量都是nil；

2.boolean 	-- 布尔 true/false

3.number：数字
可以使用科学计数法3.2e2,表示3.2*10的2次方；

4.string	-- 字符串 ' 和 " 都可以表示字符串：其字符串是不可变得值，若需要修改，则重新创建字符串
			-- 注意: lua只有nil和false是假;0，””在判断真假时，为真，
			-- \ddd 数值转换成字符；
			-- “10”+1：字符串参与运算会自动字符串转化成number；
[[ ]] 		-- 存放多行字符串
..			-- 字符串连接符(操作符左右有数字会自动将数值转换为字符串)

5.table		-- 表 是一个对象(hash)，通过构造表达式创建

t ={}    	-- 创建了一个table，Key,value 形式 
#t     		-- table最大索引值(数组部分)

6.function	-- 函数 第一类值，可以将函数名作为变量使用，或作为参数，或作为返回值；
7.userdata	-- 自定义类型
8.thread	-- 线程
...
--------------------------六.函数进阶:f=function() 等价于 function f()--------------------------
...
--------------------------------------30.线程和状态----------------------------------------
1.lua不支持真正的线程,即不支持内存共享的抢先式(preemptive)多线程;
--不支持的两个原因:
a.ANSI C没有这种功能,且没有可移植的方法能实现这种机制;
b.lua引入多线程并不是一个不好的选择;
